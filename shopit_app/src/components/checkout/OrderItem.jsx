  ...
  return (
    <div className='d-flex justify-content-between align-items-center mb-3' style={{padding : '10px', border : '1px solid #ccc', borderRadius : '5px'}}>
      <div className='d-flex align-items-center'>
        <img 
            src={`${(BASE_URL || "").replace(/\/$/, '')}/${(item.product?.image || "default.jpg").replace(/^\//, '')}`} className='img-fluid' alt={item.name} style={{width : '50px', height : '50px', objectFit : 'cover', marginRight : '10px'}} />
        <div>
